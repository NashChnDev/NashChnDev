...
class joiningPersonal extends globalclass {
    constructor(id, basic = {}, country = {}, plants = {}, departments = {}) {
        //console.log(id);
        //console.log(basic);
        super();
        this.profileimage();
    ...
    async selfpromotion() {
        let self = this;
        self.P_PP_DB = await self.promotionsavedetails();
        if (self.P_PP_DB.status == "200") {
            self.P_PP_DB = self.P_PP_DB.data.details["0"];
        }
    }
    ...
    async personaldetailsbasicdata() {
        ...
        <li class="list-inline-item col-md-2"><b>DOB</b></li>
    ...
    async employeemanagetemplateloading(){
    ...
    defaultmethod() {
        ...
        $("body").on("change","#if_promotion",async function(){
        ...
        $("body").on("click", "#employeemanage_modal_edit", async function (e) {
            console.log('hi');
            e.preventDefault();
            //console.log(self.url);
            ...
            let imgdiv = `<div style="min-width:766px;min-height:214px;text-align:center;"><img style="margin-top:10%" src="${self.url}img/loader.gif") }}"></div>`;
            $(".modal-body").html(imgdiv);
            $(".modal-title").html("Employee Management Details");
            $(".modalclose").attr("id", "employeemanageclose");
            $("#persModal").modal("show");
            let dropdownlist = await self.dropdownslist();
            let div = await self.employeemanage(dropdownlist);
            $(".modal-body").html(div);
            $(".datepicker").datepicker({
                dateFormat: "yy-mm-dd",
                changeMonth: true,
            ...
            self.activaTab("basic");
            $("input").attr("autocomplete", "off");
            await self.triggeremployeemanage();
        });
        ...
        $("body").on("click", "#personal_family", async function () {
            ...
            $("#TextBoxContainer > tr").each(function (index, tr) {
                ...
                temp.dob = $(this).find(".dob").val();
                temp.contact = $(this).find(".contact").val();
                temp.occuption = $(this).find(".occuption").val();
                temp.emergency = $(this).find(".emg").val();
        ...
        $("body").on("click", "#save_rating_details", async function (e) {
            
            let parentid = $("#basic");
            parentid.find("#error").html("");
            parentid.find("#success").html("");
            
            let ifpromotion = $("#if_promotion").val();
            let idelement = []; let process = 0;
            let data = {};
            if(ifpromotion == 'yes'){
                idelement = [
                    { id: "rating_performance", label: "Rating Perform", required: 1 },
                    { id: "rating_value", label: "Rative Value", required: 1 },
                    { id: "if_promotion", label: "Promotion", required: 1 },
                    { id: "gross_salary", label: "Gross Salary", required: 0 },
                    { id: "ctc_salary", label: "CTC Salary", required: 0 },
                    { id: "bonus", label: "Bonus", required: 0 },
                    { id: "revision_date", label: "Revision Date", required: 0 },
                    { id: "effective_date", label: "Effective Date", required: 0 },
                    { id: "promotion_date", label: "Promotion Date", required: 0 },  
                    { id: "change_designation", label: "Desgination", required: 0 },    
                    { id: "change_functions", label: "functions", required: 0 },    
                    { id: "change_department", label: "Department", required: 0 },  
                    { id: "change_plant", label: "Plant", required: 0 },  
                    { id: "reporting_manager", label: "Reporting Manager", required: 0 },                     
            ...
            if (process == 1) {
                ...
                data.temp_empid = self.P_B_DB.id;
                let promotion_id = $("#save_rating_details").data('promotion_id');
                if(promotion_id != ''){
                    data.id = promotion_id;
                }
                let savemanage = await self.employeratingsave(data);
                if (savemanage.status == "200") {
                    self.getmanagelist = await self.getpromotionlist();
                      $.each(idelement, function (key, value) {
                        parentid.find(`#${value.id}`).val("");
                    });
                    $("#save_rating_details").data('promotion_id' , '');
                    self.successmethod(
                        "basic",
                        "Rating Details Successfully Added"
                    );
                    // let prom = await self.selfpromotion();
                    // console.log(prom);
                    
                    //$("#employeemanagelist").append(temp);  
        ...
        $("body").on("click",".promotion_populate",async function(){
           let managelist = self.getmanagelist;
           let parentid = $("#basic");
           let dataid = $(this).data('promotion');
           $("#save_rating_details").data("promotion_id", dataid);

            $.each(managelist.data.details, function(key,val) {
                if(val.id == dataid){
                    let list_of_id = ['rating_performance','rating_value','if_promotion','gross_salary','ctc_salary','bonus','revision_date','effective_date'];
                    let promotion_det = ['promotion_date','change_designation','change_functions','change_department','change_plant','reporting_manager']
                    $.each(list_of_id,function(keys, vals){
                        if(val[vals] != null)
                        {
                            parentid.find(`#${vals}`).val(val[vals]);
                            if(vals == 'if_promotion'){
                                if( val[vals] == 'yes'){
                                    $("#if_promotion").trigger('change');
                                    $.each(promotion_det,function (key2,val2) {
                                        if(val[val2] != null)
                                        {
                                            parentid.find(`#${val2}`).val(val[val2]);
                                        }
                                    });
                                }else{
                                    $("#if_promotion").trigger('change');
    ...
    async promotiontriggervalue(){
        let promotion_details = await self.datapromotionvalue();
    }
        ...
        <table class="table">
          ...
          <tbody id="TextBoxContainer">
                  <tr>                                
                          ...
                          <td><input type="text" class="form-control dob datepicker"     placeholder="DOB"></td>
                          <td><select class="form-control occuption"><option value="">Select occuption</option>
                          <option value="accountant">accountant</option>
                                ...
                                <option value="saleswoman">saleswoman</option>
                          ...
                          <td><select class="form-control emg">
    ...
    async employeemanage(res){
        let self = this;
        //console.log(self.plants);
        //let dropdowndet = dropdowns;
        let dropdownlist = res.data.dropdown;
        let designationlist ='<option value="">Select designation</option>'; let plantlist = ''; let locationlist = '';
        let costcenter = ''; let funlist = '<option value="">Select Functions</option>';
        let managelist = '';
        self.getmanagelist = await self.getpromotionlist();

        let departmentlist = '';
     
        if(self.getmanagelist.data.status =='Success'){
            $.each(self.getmanagelist.data.details,function(key,val){
                let temp = `<li class="list-inline-item col-md-2">
                <button class="btn btn-link promotion_populate" data-promotion="${val.id}">Revision - ${key+1}    ${val.rating_performance}</button></li>`;
                //temp +=`<td></td>`;
        ...
        if(dropdownlist.length > 0){           
            $.each(dropdownlist,function(key,val){
                if(val.fieldsname == 'designation'){
                    designationlist +=`<option value="${val.optionvalue}">${val.optionvalue}</option>`;
                }
                if(val.fieldsname == 'Costcenter'){
                    costcenter +=`<option value="${val.optionvalue}">${val.optionvalue}</option>`;
                }
                console.log(val.fieldsname);
                if(val.fieldsname == 'functions'){
                    console.log(val.optionvalue);
                    funlist +=`<option value="${val.optionvalue}">${val.optionvalue}</option>`;
                }
            });
        }
        console.log(self.plants);
        if(self.plants.length > 0){
            $.each(self.plants,function(key,val){                
                plantlist +=`<option value="${val.plantcode}">${val.plantcode}</option>`;               
            });
        }
        if(self.departments.length > 0){
            $.each(self.departments,function(key,val){                
                departmentlist +=`<option value="${val.deptcode}">${val.deptname}</option>`;               
            });
        }
        // if(self.plants.length > 0){
        //     $.each(self.plants,function(key,val){                
        //         locationlist +=`<option value="${val.location}">${val.location}</option>`;               
        //     });
        // }

        return `<ul class="nav nav-tabs">                  
                 <li class="active"><a class="nav-item nav-link" data-toggle="tab" href="#basic">Employee Rating</a></li>   
                </ul>
        <div class="tab-content">     
            <div id="basic"  class="tab-pane fade in active" > 
                   <div class="row">
                        <div class="form-group col-md-4">
                        <label>Select peroformance Rating</label>
                            <select class="form-control" id="rating_performance">
                            <option value= ''>Select Financial Year</option>
                                ...
                                <option value="23-24">24-25</option>
                                <option value="24-25">25-26</option>
                            </select>
                        </div>
                        <div class="form-group col-md-4">
                             <label>Select Rating Out of 5</label>
                        ...
                        <div class="form-group col-md-4">
                            <label>If Promotion Details</label>
                            <select class="form-control" id="if_promotion">
                   ...
                   <label><h5><b>Salary Revision <b></h5></label>
                    <div class="row">
                        <div class="form-group col-md-4">
                            <label  class=" control-label">Gross Salary</label>
                            <input class="form-control" id="gross_salary"  name="gross_salary" autocomplete="off" type="text" placeholder="Enter Gross Salary here...">           
                        </div>
                        <div class="form-group col-md-4">
                            <label  class=" control-label">CTC Salary</label>
                            <input class="form-control"  id="ctc_salary" name="ctc_salary" autocomplete="off" type="text" placeholder="Enter CTC Salary here...">           
                        </div> 
                        <div class="form-group col-md-4">
                            <label  class=" control-label">Bonus</label>
                            <input class="form-control" id="bonus" name="bonus" autocomplete="off" type="text" placeholder="Enter File Name here...">           
                        </div>    
                        <div class="form-group col-md-4">
                        <label  class=" control-label">Revision Date</label>
                        <input class="form-control datepicker" id="revision_date" name="revision_date" autocomplete="off" type="text" placeholder="Enter Salary Revision Date here...">           
                        </div> 
                        <div class="form-group col-md-4">
                        <label  class=" control-label">Revision Effective Date</label>
                        <input class="form-control datepicker" id="effective_date" name="effective_date" autocomplete="off" type="text" placeholder="Enter Salary Revision Effective Date here...">           
                        </div>    
                    </div>
                    <label class="promotion_details" style="display:none"><h5><b>Promotion Revision <b></h5></label>
                    <div class="row promotion_details" style="display:none">                  
                            <div class="form-group col-md-4">
                                <label  class=" control-label">Promotion Date</label>
                                <input class="form-control datepicker" id="promotion_date"  name="gross_salary" autocomplete="off" type="text" placeholder="Enter Promotion Date here...">           
                            </div>
                            <div class="form-group col-md-4">
                                <label  class=" control-label">Change Designation</label>
                                <select class="form-control" id="change_designation">                       
                                    ${designationlist}
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label  class=" control-label">Change Functions</label>
                                <select class="form-control" id="change_functions">                       
                                    ${funlist}
                                </select>
                            </div>                       
                      
                            <div class="form-group col-md-4">
                                <label  class=" control-label">Change Department </label>
                                <select class="form-control" id="change_department">
                                    <option value=''>Select Departments</option>
                                    ${departmentlist}
                                </select>
                            </div>
                            <div class="form-group col-md-4">
                                <label  class=" control-label">Change Plant </label>
                                <select class="form-control" id="change_plant">                       
                            ...
                            <div class="form-group col-md-4">
                               ...
                               <input type="text" class="form-control" id="reporting_manager">
                            </div>                       
                    </div>
                   ...
                   <span id="error" style="color: red;font-size:14px"></span>
                   <span id="success" style="color:gray;font-size:14px"></span>
                   <button type="button" class="btn btn-primary"  id="save_rating_details">Save Rating Details</button>
                     </div>   
                  </div> 
                  <div class="row">
                  <ul class="list-inline col-md-12" id="list_of_promotion">
                  ${managelist}
                  </ul>
              </div>
    ...
    GetDynamicTextBox(value) {
        return `<tr><td><input type="text" class="form-control names"  placeholder="Name"></td>
                       <td><select class="form-control relationship">
                       <option value="">Select Relationship</option>
                       <option value="Father">Father</option>
                       <option value="Mother">Mother</option>
                       <option value="Son">Son</option>
    ...
    // async employeemanagesave(data) {
    //     return await axios.post(
    //         `${this.url}joiningform/savemanage`,
    //         data
    //     );
    // }
    async employeratingsave(data) {
        return await axios.post(
            `${this.url}joiningform/saverating`,
            data
        );
    }
    ...
    async promotionsavedetails() {
        let self = this;
        let temp_empid = self.P_B_DB.id;
        return await axios.get( `${this.url}joiningform/getpromotiondetails/` + temp_empid)
    }
    ...
    async getpromotionlist() {
        ...
        let temp_empid = self.P_B_DB.id;
        return await axios.get(
            `${this.url}joiningform/getpromotionsdetails/` + temp_empid
        );
    }
    ...
    async dropdownslist(){
        return await axios.get(`${this.url}joiningform/dropdowns`);
    }
...
class joiningindex extends globalclass {
    ...
    defaultmethod() {
        ...
        $(".saveemployee").on("click", async function (e) {
            ...
            if (parentid.find("#name").val() == "") {
                ...
                data.area = parentid.find("#area").val();
                data.sub_area   = parentid.find("#sub_area").val();
                data.functions = parentid.find("#functions").val();
                data.cost_center = parentid.find("#cost_center").val();
                data.gross_salary = parentid.find("#gross_salary").val();
                data.ctc_salary = parentid.find("#ctc_salary").val();
                data.bonus = parentid.find("#bonus").val();
        ...
        $("body").on("change", "#department", async function (e) {
            console.log('hi');
            e.preventDefault();
            let thisvalue = $(this).val();
            let plantdetails = await self.getarea(thisvalue);
            //console.log(plantdetails);
            let departmentoption = `<option value="">Select Area</option>`;
            if (plantdetails.status == 200) {
                if (plantdetails.data.area != undefined) {
                    $.each(plantdetails.data.area, function (key, val) {
                        departmentoption += `<option value=${val.areacode}>${val.areaname}</option>`;
                    });
                }
            }
            $("#area").html(departmentoption);
        });
        $("body").on("change", "#area", async function (e) {
            console.log('hi');
            e.preventDefault();
            let thisvalue = $(this).val();
            let plantdetails = await self.getsubarea(thisvalue);
            //console.log(plantdetails);
            let departmentoption = `<option value="">Select SubArea</option>`;
            if (plantdetails.status == 200) {
                if (plantdetails.data.subarea != undefined) {
                    $.each(plantdetails.data.subarea, function (key, val) {
                        departmentoption += `<option value=${val.subareacode}>${val.subareaname}</option>`;
                    });
                }
            }
            $("#subarea").html(departmentoption);
        });
    ...
    async getarea(data){
        let self = this;
        return await axios.get(`${this.url}joiningform/getarea/` + data);
    }
    async getsubarea(data){
        let self = this;
        return await axios.get(`${this.url}joiningform/getsubarea/` + data)
    }
    async getemployeefiles(id) {
        return await axios.get(`${this.url}joiningform/getemployeefiles/` + id);
    }
...
class dashboardhome extends globalclass {
    constructor(pielist = {}, activelist = {}, employeetype = {}) {
        super();

        this.piedata = JSON.parse(pielist);
        this.activedata = JSON.parse(activelist);
        this.employeetype = JSON.parse(employeetype);
        this.defaultmethod();
    ...
    defaultmethod() {
        ...
        if (
            ...
            Object.keys(this.activedata).length > 0
        ) {
            $(".activeimg").hide();
        ...
        if (
            ...
            Object.keys(this.employeetype).length > 0
        ) {
            $(".emptypeimg").hide();
            $.each(this.employeetype, function (key, val) {
                emptype.push(key);
                empval.push(val);
                backgroundcolor.push(dynamicColors());
            });
            var empctx = document.getElementById("empChart");

            var empChart = new Chart(empctx, {
                type: "bar",
                data: {
                    labels: emptype,
                    datasets: [
        ...
        if (
            ...
            Object.keys(this.piedata).length > 0
        ) {
            $(".headimg").hide();
            $.each(this.piedata, function (key, val) {
                departments.push(key);
                departcount.push(val);
                backgroundcolor.push(dynamicColors());
            });
